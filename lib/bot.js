...
import { genderCtrl, aboutCtrl, setServiceCtrl, setGenderCtrl, addPhotoCtrl, userPhotoCtrl, removePhotoCtrl } from "./controllers/userControllers.js";
...
bot.command('start', async ctx => await renderMainMenu(ctx));
bot.hears(/.*/, async ctx => {
  const {
    search,
    profile,
  ...
  logger.log('HEAR:', text); // menu button handlers
...
bot.action(/.*/, async ctx => {
  ...
  if (actionName === actions.removePhotoList) return await removePhotoCtrl(ctx, actionParams);
});
bot.on('message', ctx => {
  if (inputManager.handleQuestion(ctx)) return;
});
