...
export const renderInputQuestion = async (ctx, text, questionId, handler = () => null) => {
...
export class QuestionManager {
  ...
  getAndRemoveUserQuestion(userId) {
  ...
  setUserQuestion(userId, question, handler) {
    if (question && userId) {
      const date = new Date();
      this.pendingInputs[userId] = {
        question,
        date,
        handler
      };
